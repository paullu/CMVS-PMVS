...
void PhotoSetS::init(const std::vector<int>& images, const std::string prefix,
...
void PhotoSetS::free(void) {
  for (int index = 0; index < (int)m_photos.size(); ++index)
    m_photos[index].free();
}

void PhotoSetS::free(const int level) {
  for (int index = 0; index < (int)m_photos.size(); ++index)
    m_photos[index].free(level);
}

void PhotoSetS::setEdge(const float threshold) {
  for (int index = 0; index < m_num; ++index)
    m_photos[index].setEdge(threshold);
}

void PhotoSetS::write(const std::string outdir) {
  for (int index = 0; index < m_num; ++index) {
    const int image = m_images[index];
    char buffer[1024];
...
void PhotoSetS::getPAxes(const int index, const Vec4f& coord, const Vec4f& normal,
                          Vec4f& pxaxis, Vec4f& pyaxis) const{
  m_photos[index].getPAxes(coord, normal, pxaxis, pyaxis);
}

void PhotoSetS::grabTex(const int index, const int level, const Vec2f& icoord,
...
void PhotoSetS::grabTex(const int index, const int level, const Vec4f& coord,
...
float PhotoSetS::incc(const std::vector<std::vector<Vec3f> >& texs,
...
void PhotoSetS::getMinMaxAngles(const Vec4f& coord, const std::vector<int>& indexes,
...
int PhotoSetS::checkAngles(const Vec4f& coord,
...
float PhotoSetS::computeDepth(const int index, const Vec4f& coord) const {
  return m_photos[index].computeDepth(coord);
}

void PhotoSetS::setDistances(void) {
...
int PhotoSetS::image2index(const int image) const {
