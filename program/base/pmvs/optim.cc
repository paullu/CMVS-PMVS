...
Optim::Optim(FindMatch& findMatch) : m_fm(findMatch) {
  m_one = this;
...
int Optim::preProcess(Cpatch& patch, const int id, const int seed) {
  addImages(patch);
...
void Optim::filterImagesByAngle(Cpatch& patch) {
...
int Optim::postProcess(Cpatch& patch, const int id, const int seed) {
  if ((int)patch.m_images.size() < m_fm.m_minImageNumThreshold)
    return 1;
...
void Optim::constraintImages(Cpatch& patch, const float nccThreshold,
                              const int id) {
  vector<float> inccs;
...
void Optim::setRefImage(Cpatch& patch, const int id) {
#ifdef DEBUG
...
void Optim::setRefConstraintImages(Cpatch& patch, const float nccThreshold,
                                    const int id) {
  //----------------------------------------------------------------------
  // Set the reference image
  vector<vector<float> > inccs;
...
void Optim::sortImages(Cpatch& patch) const{
  const int newm = 1;
  if (newm == 1) {
...
void Optim::removeImagesEdge(Patch::Cpatch& patch) const{
  vector<int> newindexes;
...
void Optim::refinePatch(Cpatch& patch, const int id,
                         const int time) {
...
bool Optim::refinePatchBFGS(Cpatch& patch, const int id,
...
void Optim::normalize(std::vector<std::vector<float> >& texs,
                       const int size) {
  // compute average rgb
  Vec3f ave;
  int denom = 0;
...
float Optim::ssd(const std::vector<float>& tex0,
		  const std::vector<float>& tex1) const{
...
void Optim::setWeightsT(const Patch::Cpatch& patch, const int id) {
  computeUnits(patch, m_weightsT[id]);
  for (int i = 1; i < (int)m_weightsT[id].size(); ++i)
    m_weightsT[id][i] = min(1.0f, m_weightsT[id][0] / m_weightsT[id][i]);  
  m_weightsT[id][0] = 1.0f;
